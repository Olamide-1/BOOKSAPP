...
def book_search(request):
    ...
    params = {
        'q': query,
        'key': 'AIzaSyDMCyrg_IBCWFJh3VQL_p6s3XWSQ1gIqvQ', # replace with your actual API key
    }
    ...
    for item in data.get('items', []):
        book = {
            ...
            'image_url': item['volumeInfo']['imageLinks']['thumbnail'] if 'imageLinks' in item['volumeInfo'] else None,
            'preview_url': item['volumeInfo']['previewLink'],
        }
        books.append(book)
    return render(request, 'publisher/book_search.html', {'books': books})




def book_events(request):
    url = 'http://lastes.eba-xcccgp4b.us-east-1.elasticbeanstalk.com/eventlist/books'
    response = requests.get(url)
    events = response.json()

    return render(request, 'publisher/book_events.html', {'events': events})
